...
var fs = require('fs');
var url_dict = require(path.join(__dirname, '../', config.urls));
...
Object.keys(url_dict).forEach(scene => {
    ...
    if (config.scenes && config.scences.indexOf(scene) == -1) return;

    var aos = config.actionsOfScenes[scene];
    var out_dir = path.join(config.out_dir, scene);
    Object.keys(urls).forEach(action => {
        if (aos != null && aos.indexOf(action) == -1) return;
