...
if (config.benchtool) {
    bench_tool = config.benchtool;
}

var TPL = fs.readFileSync(path.join(__dirname, 'test-cmd-tpl.' + bash_cmd)).toString();
var URL_CMD_TPL = fs.readFileSync(path.join(__dirname, 'url-cmd-tpl@' + bench_tool + '.' + bash_cmd)).toString();
...
Object.keys(url_dict).forEach(scene => {
    ...
    Object.keys(urls).forEach(uk => {
        ...
        var options = ['timelimit', 'count', 'variable', 'cmd_options']
        options.forEach(ops => {
